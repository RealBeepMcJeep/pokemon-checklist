...
A dependency-free wild Pokémon checklist for the supplied Generation VII location guide. It covers National Dex 001–807, chronological island/location sections, encounter rates, numbered grass maps, SOS allies, optional forms, and a searchable Pokédex.
...
Status buttons cycle **None → Caught → Seen → None**. Only Caught counts toward location and overall progress. The browser saves species statuses, form statuses, and the Form Dex setting in versioned local storage (`pokemon-checklist-state-v1`). Export creates deterministic schema-version-1 JSON; Import validates the complete file before asking to replace the current state. If storage is unavailable, changes remain visible in memory for the tab.
...
To regenerate the extracted encounter data and PDF images, use `python tools/extract_reference.py --extract-assets`. To refresh the pinned Pokémon name snapshot as well, add `--refresh-pokemon`. Run `python tools/build.py` afterward; `--check` verifies canonical schemas, references, stable IDs, categories, assets, atlas dimensions, embedded-resource completeness, script-safe escaping, external-resource absence, and generated `index.html` freshness.
